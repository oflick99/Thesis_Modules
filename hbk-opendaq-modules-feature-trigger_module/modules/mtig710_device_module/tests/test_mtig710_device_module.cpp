...
#include "mtig710_device_module_factories.h"

using Mtig710DeviceModule = testing::Test;

using namespace daq;
using namespace lanfactory;

TEST_F(Mtig710DeviceModule, CreateModule)
{
...
TEST_F(Mtig710DeviceModule, ModuleName)
{
    auto module = CreateModule();
    ASSERT_EQ(module.getModuleInfo().getName(), "Mtig710DeviceModule");
}

TEST_F(Mtig710DeviceModule, VersionAvailable)
{
    auto module = CreateModule();
    ASSERT_TRUE(module.getModuleInfo().getVersionInfo().assigned());
}

TEST_F(Mtig710DeviceModule, EnumerateDevices)
{
    auto module = CreateModule();
...
TEST_F(Mtig710DeviceModule, GetAvailableComponentTypes)
{
    ...
    {
        ASSERT_EQ(deviceType.first, "Mtig710");
        ASSERT_EQ(deviceType.second.getId(), "Mtig710");
        ASSERT_EQ(deviceType.second.getName(), "Mtig710 device");
...
TEST_F(Mtig710DeviceModule, AddDevice)
{    
    ...
    ASSERT_EQ(device.getChannels().getCount(), 1);
