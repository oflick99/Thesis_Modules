...
DictPtr<IString, IFunctionBlockType> TriggerFbModule::onGetAvailableFunctionBlockTypes()
{
    ...
    const auto logic = LogicGateFbImpl::CreateType();
                                                    ...
                                                    const PropertyObjectPtr& config)
{
    ...
    if (id == LogicGateFbImpl::CreateType().getId())
    {
        FunctionBlockPtr fb = createWithImplementation<IFunctionBlock, LogicGateFbImpl>(context, parent, localId, config);
        return fb;
    }
